...
/**
 * Created by RAMMURALI on 4/6/2017.
 */
        ...
        DialogInterface.OnMultiChoiceClickListener
{
    ...
    public void setItems(List<String> items,String[] itemscode) {
        _itemscode = itemscode;
    ...
    private String buildSelectedItemString() {
        ...
        for (int i = 0; i < _items.length; ++i) {
            if (mSelection[i]) {
                ...
                sb.append(_items[i]);
                sbcode.append(_itemscode[i]);
                try {
                    JSONObject jobj = new JSONObject();
                    jobj.put("answer_id", _itemscode[i]);
                    jobj.put("answer", _items[i]);
        ...
        String aa = this.getTag().toString();
        try {
            for (int k = 0; k < formarray.length(); k++) {
                ...
                for (int l = 0; l < questionsarray.length(); l++) {
                    if (questionsarray.getJSONObject(l).getString("unique_id").toString().equalsIgnoreCase(this.getTag().toString())) {
